...
<template>
	<div v-if="isLoadnigShipped" class="text-center">Loading orders...</div>

	<!-- Error State -->
	<div v-else-if="isErrorShipped" class="text-center text-red-500">
		There was an error loading the orders. Please try again.
	</div>
	
	<div v-else-if="shippedOrders && shippedOrders?.length >= 1" id="order-list"
